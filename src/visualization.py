...
monthly_df.rename(columns={'數值/Value': '月降雨量'}, inplace=True)
...
FRAMES_PER_MONTH = 30  # 每月显示帧数，1s过渡
def lerp(a, b, t):
    return a + (b - a) * t
...
def animate_func(frame):
    ...
    rain1 = float(row1['月降雨量'])
    rain2 = float(row2['月降雨量'])
    ...
    k = lerp(k1, k2, t)
    # 圆整体大小随降雨量变化，降雨量小整体更小
    min_scale = 0.65  # 最小缩放比例
    max_scale = 1.0   # 最大缩放比例
    ...
    # 内圈：降雨量小为白色，大为蓝紫色渐变
    bluepurple = (106/255, 90/255, 205/255)  # 蓝紫色
